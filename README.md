...
> 최종 제출 모델은 전체 Val 점수가 아니라 **대회 원본 도메인만으로 재평가한 결과**를 기준으로 선택했습니다. (아래 "알려진 한계" 참고)
...
| 결정 | 근거 요약 |
|---|---|
| **AI-Hub 데이터 추가** | 대회 원본 232장으로는 118클래스 학습이 어려워 12,196장으로 확대 |
| **YOLO 3종 비교** (11s / 11m / 12m) | 모델 크기 축과 아키텍처 세대 축을 동시에 비교하기 위해 |
| **단일 데이터셋 공유** | 모델별 데이터가 다르면 성능 차이의 원인을 분리할 수 없어, SHA-256 대조로 동일성 강제 |
| **입력 해상도 960** | 640에서는 알약 각인이 뭉개져 판독 불가 — EDA의 해상도 진단 결과 |
| **기존 Train/Val 분할 보존** | 이미 그룹 안전 분할이었고(누수 0건), 재분할 시 모델 간 비교가 깨지므로 |
| **학습과 평가 분리** | 9개 실험 학습에 수십 시간 소요 — Colab 세션 제한 대응 |
| **audit / release 2-pass** | 품질 게이트 통과 시에만 산출물 생성해, 검증 안 된 데이터 배포를 구조적으로 차단 |
...
로컬 Validation 0.97과 실제 Kaggle 0.46의 격차 원인을 다섯 개 가설로 세우고 각각 실험으로 검증했습니다.

| 가설 | 검증 방법 | 결과 |
|---|---|---|
| 유령 클래스가 mAP 분모를 희석 | 56클래스만 남긴 CSV 제출 | 기각 |
| GT 클래스 수가 많아 점수 상한이 존재 | 리더보드 상위 점수 확인 | 기각 |
| confidence 0.001이 잘못 선택됨 | 대회 원본 도메인으로 재탐색 | 기각 |
| Top-K 과다 검출 | confidence 순위별 분포 분석 | 기각 |
| 좌표 역변환 오류 | forward / inverse 수식 대조 | 기각 |

**핵심 발견** — Validation 2,433장 중 대회 원본 분포는 56장에 불과해, **지표를 정확히 구현했음에도 측정 대상이 대표성을 갖지 못했습니다.** 지표를 올바르게 구현하는 것과 올바르게 측정하는 것은 별개의 문제였습니다.
...
1. **Validation 대표성** — Validation 2,433장 중 대회 원본 분포는 56장뿐이라, 로컬 지표(0.97)가 실제 Kaggle 성능(0.47)을 반영하지 못했습니다. 최종 모델 선택은 원본 도메인만 재평가한 결과를 사용했습니다.
2. **근접중복 스캔 미완료** — 후보 상한에서 중단되어 Train/Test 누수 검사가 완전하지 않습니다. (`near_duplicate_scan_complete: false`)
3. **육안 수동 검토 생략** — 일정상 769건을 유형별 자동 정책으로 승인했으며, 판정 근거는 `manual_review_decisions_v6_0.csv`의 `note` 컬럼에 기록되어 있습니다.
4. **전처리 영향 미검증** — 전처리를 제외한 베이스라인과의 비교는 전체 재학습이 필요해 수행하지 못했습니다.
