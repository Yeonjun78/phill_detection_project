...
| **입력 해상도 960** | `target_resolution_bbox_diagnostics.csv`에서 640/960/1280의 BBox 짧은 변 픽셀 크기를 비교. 640에서는 알약 각인이 뭉개짐 |
| **촬영 그룹 단위 분할** | 동일 알약 조합을 여러 각도로 촬영한 구조라 무작위 분할 시 성능 과대평가. 4,104개 그룹 전부 누수 0건 검증 |
| **전처리 순서** | EXIF → ICC/sRGB → bilateral denoise → clipped gray-world WB → LAB L채널 CLAHE → letterbox. 색상 왜곡 없이 각인 대비만 강화 |
| **audit / release 2-pass** | 품질 게이트를 통과한 경우에만 최종 산출물 생성. 검증 없는 데이터 배포를 구조적으로 차단 |
| **재현성 지문** | `group_contract_sha256`, `label_contract_sha256`, `config_sha256` 기록으로 동일 입력 사용 여부를 바이트 수준 대조 |
...
| 문서 | 내용 |
|---|---|
| [프로젝트 평가항목별 정리](reports/프로젝트_평가항목별_정리.md) | 데이터·모델·평가·코드품질 항목별 수행 내역과 보완점 |
| [Kaggle 점수 원인 진단](reports/Kaggle_0.46점_원인진단_및_파일별_책임소재.md) | 로컬 0.973 ↔ 실제 0.46 격차에 대한 가설·검증·기각 기록 |
...
1. **근접중복 스캔 미완료** — 후보 상한 3,000건에서 중단되어 Train/Test 누수 검사가 완전하지 않습니다. (`near_duplicate_scan_complete: false`)
2. **육안 수동 검토 생략** — 마감 일정상 769건을 유형별 자동 정책으로 승인했습니다. 판정 근거는 `manual_review_decisions_v6_0.csv`의 `note` 컬럼에 기록되어 있습니다.
3. **Validation 대표성** — 위 "가설 검증 기록" 참고.
...
- 데이터, 체크포인트, 대용량 산출물은 `.gitignore` 로 추적 제외되어 있습니다.
